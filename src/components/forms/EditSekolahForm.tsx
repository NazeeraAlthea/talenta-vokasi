...
export default function EditSekolahForm({ school }: FormProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // 2. Kirim semua data (termasuk URL logo baru) ke API Route
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-1"><label htmlFor="level" className="block text-sm font-medium text-gray-700">Jenjang</label><input name="level" id="level" value={formData.level} onChange={handleInputChange} className={disabledInputStyle} disabled></input></div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="mt-2">
                    {!logoFile ? (
                        <div className="border-2 border-dashed rounded-lg p-10 text-center bg-gray-50 hover:border-indigo-500" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); handleFileSelect(e.dataTransfer.files) }}>
            ...
            <div className="flex justify-end gap-4 pt-6 border-t">
                <button type="button" onClick={() => router.push('/sekolah/profil')} className="rounded-md bg-white px-4 py-2 text-sm font-medium text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Batal</button>
                <button onClick={handleSubmit} disabled={isSaving} className="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 disabled:bg-gray-400">
