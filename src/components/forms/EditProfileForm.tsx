...
import { FileText, UploadCloud, Loader2, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        <div>
          ...
          <input type="text" name="full_name" id="full_name" value={formData.full_name} onChange={handleInputChange} className={inputStyle} />
        </div>
        <div>
          <label htmlFor="school_id" className="block text-sm font-medium leading-6 text-gray-900">Asal Sekolah</label>
          <select name="school_id" id="school_id" value={formData.school_id} className={disabledInputStyle} disabled>
            {schools.map(school => <option key={school.id} value={school.id}>{school.name}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="major_id" className="block text-sm font-medium leading-6 text-gray-900">Jurusan</label>
          <select name="major_id" id="major_id" value={formData.major_id} className={disabledInputStyle} disabled>
            {majors.map(major => <option key={major.id} value={major.id}>{major.name}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="portfolio_url" className="block text-sm font-medium leading-6 text-gray-900">Link Portofolio (Opsional)</label>
          <input type="url" name="portfolio_url" id="portfolio_url" placeholder="https://..." value={formData.portfolio_url} onChange={handleInputChange} className={inputStyle} />
        </div>
      </div>
      ...
      {/* <-- 3. Pesan Error dengan gaya yang disempurnakan --> */}
      {error && 
      ...
      <div className="flex justify-end gap-4 pt-6 border-t">
        ...
        <button onClick={handleSubmit} disabled={isSaving} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 disabled:bg-indigo-300 disabled:cursor-not-allowed">
